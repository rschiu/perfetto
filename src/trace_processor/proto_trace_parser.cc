...
SystraceParseResult ParseSystraceTracePoint(base::StringView str,
  ...
  if (len < 2)
    return SystraceParseResult::kParsingFailure;
  ...
  if (s[0] != 'B' && s[0] != 'E' && s[0] != 'C') {
    return s[0] == 'S' || s[0] == 'F'
               ? SystraceParseResult::kTracePointUnsupported
               : SystraceParseResult::kParsingFailure;
  }
  ...
  switch (s[0]) {
    case 'B': {
      ...
      return SystraceParseResult::kParsingSuccess;
    }
    case 'E': {
      return SystraceParseResult::kParsingSuccess;
    }
...
void ProtoTraceParser::ParsePrint(uint32_t,
  ...
  if (r != SystraceParseResult::kParsingSuccess) {
    if (r == SystraceParseResult::kParsingFailure) {
