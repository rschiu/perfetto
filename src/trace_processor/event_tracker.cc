...
RowId EventTracker::PushCounter(int64_t timestamp,
  ...
  if (resolve_utid_to_upid) {
    // TODO(lalitm): this is only valid if we are guaranteed that at flush
    // time we will resolve this row properly. Otherwise, we can end up with
    // this row being unjoinable.
    defn_id = std::numeric_limits<uint32_t>::max();
  } else {
...
RowId EventTracker::PushInstant(int64_t timestamp,
  ...
  if (resolve_utid_to_upid) {
    // TODO(lalitm): this is only valid if we are guaranteed that at flush
    // time we will resolve this row properly. Otherwise, we can end up with
    // this row being associated with upid == 0.
...
void EventTracker::FlushPendingEvents() {
  ...
  for (const auto& pending_counter : pending_upid_resolution_counter_) {
    size_t row = pending_counter.row;
    ...
    context_->storage->mutable_counter_values()->set_counter_id(row, id);
  }
