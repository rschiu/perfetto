...
void InternMessage(TraceProcessorContext* context,
  ...
  uint32_t iid = 0;
  protozero::ProtoDecoder decoder(message.data(), message.length());
  ...
  PERFETTO_DCHECK(res.second ||
                  (res.first->second.message.length() == message.length() &&
                   memcmp(res.first->second.message.data(), message.data(),
                          message.length()) == 0));
}
...
void ProtoTraceTokenizer::ParsePacket(TraceBlobView packet) {
  ...
  if (decoder.has_track_event()) {
    auto field = decoder.thread_descriptor();
    const size_t offset = packet.offset_of(field.data);
    ParseTrackEventPacket(decoder, packet.slice(offset, field.size));
    return;
  }
...
void ProtoTraceTokenizer::HandleIncrementalStateCleared(
    protos::pbzero::TracePacket::Decoder& packet_decoder) {
...
void ProtoTraceTokenizer::ParseThreadDescriptorPacket(
  ...
  state->SetThreadDescriptor(thread_descriptor_decoder.pid(),
                             thread_descriptor_decoder.tid());
  state->SetTrackEventTimeNs(
      thread_descriptor_decoder.reference_timestamp_us() * 1000);
  state->SetTrackEventThreadTimeNs(
...
void ProtoTraceTokenizer::ParseTrackEventPacket(
  ...
  // TODO(eseckler).
  ProtoDecoder event_decoder(event.data(), event.length());
  ...
  if (auto ts_delta_field =
    ...
    timestamp =
        state->IncrementAndGetTrackEventTimeNs(ts_delta_field.as_int64());
  } else if (auto ts_absolute_field =
                 event_decoder.FindField(kTimestampAbsoluteUsFieldNumber)) {
    timestamp = ts_absolute_field.as_int64();
  } else {
  ...
  if (auto tt_delta_field =
    ...
    thread_timestamp =
        state->IncrementAndGetTrackEventThreadTimeNs(tt_delta_field.as_int64());
  } else if (auto tt_absolute_field =
                 event_decoder.FindField(kThreadTimeAbsoluteUsFieldNumber)) {
    thread_timestamp = tt_absolute_field.as_int64();
  }

  context_->sorter->PushTrackEvent(timestamp, thread_timestamp, state,
                                   std::move(event));
}
