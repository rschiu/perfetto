...
int TraceProcessorMain(int argc, char** argv) {
  ...
  auto t_load_start = base::GetBootTimeNs();
  for (int i = 0;; i++) {
    if (i % 128 == 0)
  ...
  auto t_load = base::GetBootTimeNs() - t_load_start;
  ...
  auto t_run_start = base::GetBootTimeNs();
  ...
  if (metric_names) {
    ...
    if (!ret) {
      auto t_query = base::GetBootTimeNs() - t_run_start;
