...
void HeapProfileTracker::AddMapping(ProfileIndex pidx,
  ...
  if (raw_build_id_str.size() > 1) {
    std::string hex_build_id =
        ToHex(raw_build_id_str.c_str(), raw_build_id_str.size() - 1);
    build_id = context_->storage->InternString(
        {hex_build_id.data(), hex_build_id.size()});
  }
