...
class TraceFormatWriter {
 public:
  TraceFormatWriter(std::ostream& output)
  ...
  void WriteSlice(int64_t begin_ts_us,
    ...
    value["duration"] = Json::Int64(duration_us);
    output_ << writer.write(value);
    first_event_ = false;
  }
 ...
 private:
  void WriteHeader() { output_ << "{\"traceEvents\":[\n"; }

  void WriteFooter() { output_ << "]}" << std::endl; }
...
void ExportJson(const TraceStorage* storage, std::ostream& output) {
  ...
  for (size_t i = 0; i < slices.slice_count(); ++i) {
    if (slices.types()[i] == RefType::kRefUtid) {
      ...
      // TODO: how to determine tid and pid if reference type is not utid?
    }
  }
}
