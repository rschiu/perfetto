...
class HeapprofdEndToEnd : public ::testing::Test {
 ...
 protected:
  ...
  void NativeStartupDenormalized() {
    auto helper = GetHelper(&task_runner);
  ...
  void DiscoverByNameDenormalized() {
    auto helper = GetHelper(&task_runner);
...
TEST_F(HeapprofdEndToEnd, NativeStartupDenormalized_Central) {
  if (IsX86())
    return;

  auto prop = DisableFork();
  ASSERT_EQ(ReadProperty(kHeapprofdModeProperty, ""), "");
  NativeStartupDenormalized();
}
...
TEST_F(HeapprofdEndToEnd, DiscoverByNameDenormalized_Central) {
  if (IsX86())
    return;

  auto prop = DisableFork();
  ASSERT_EQ(ReadProperty(kHeapprofdModeProperty, ""), "");
  DiscoverByNameDenormalized();
}

TEST_F(HeapprofdEndToEnd, NativeStartupDenormalized_Fork) {
  if (IsX86())
    return;
