...
constexpr uint32_t kChildHeapprofdHeartbeatCheckPeriodMs = 10 * 1000;
...
void HeapprofdProducer::ScheduleHeartbeatCheck() {
  PERFETTO_DCHECK(mode_ == HeapprofdMode::kChild);
  ...
  task_runner_->PostDelayedTask(
      [weak_producer]() {
        if (!weak_producer)
          return;
        weak_producer->VerifyActiveOrExitPeriodic();
      },
      kChildHeapprofdHeartbeatCheckPeriodMs);
}

void HeapprofdProducer::VerifyActiveOrExitPeriodic() {
  PERFETTO_DCHECK(mode_ == HeapprofdMode::kChild);
