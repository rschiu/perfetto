...
int FuzzRingBufferWrite(const uint8_t* data, size_t size) {
  ...
  // pages).
  size_t payload_size = size - sizeof(DataHeader);
  ...
  MetadataHeader& metadata_header = header.metadata_header;
  metadata_header.spinlock = 0;

  PERFETTO_CHECK(ftruncate(*fd, static_cast<off_t>(total_size_pages *
                                                   base::kPageSize)) == 0);
  PERFETTO_CHECK(
      base::WriteAll(*fd, &metadata_header, sizeof(metadata_header)) != -1);
