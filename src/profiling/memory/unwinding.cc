...
constexpr size_t kUnwindChunkSize = 1000;
...
void UnwindingWorker::OnDataAvailable(base::UnixSocket* self) {
  ...
  char recv_buf[kUnwindChunkSize];
  self->Receive(recv_buf, sizeof(recv_buf));
  HandleBufferChunk(self->peer_pid());
}

void UnwindingWorker::HandleBufferChunk(pid_t peer_pid) {
  ...
  if (i == kUnwindChunkSize) {
    thread_task_runner_.get()->PostTask(
        [this, peer_pid] { HandleBufferChunk(peer_pid); });
  }
}
