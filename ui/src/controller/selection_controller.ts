...
export class SelectionController extends Controller<'main'> {
      ...
      this.args.engine.query(sqlQuery).then(result => {
        ...
        if (result.numRecords === 1 &&
          ...
          this.schedulingDetails(ts, utid).then(wakeResult => {
            selected.wakeupTs =
                wakeResult ? fromNs(wakeResult.wakeupTs) : undefined;
            selected.wakerUtid = wakeResult ? wakeResult.wakerUtid : undefined;
            selected.wakerCpu = wakeResult ? wakeResult.wakerCpu : undefined;
  ...
  async schedulingDetails(ts: number, utid: number|Long) {
    ...
    if (wakeupTs) {
      ...
      if (wakerRow) {
        return {wakeupTs, wakerUtid: wakerRow[0], wakerCpu: wakerRow[1]};
      } else {
        return undefined;
      }
    }
