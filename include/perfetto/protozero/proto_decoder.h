...
// It does NOT recurse in nested submessage, instead it just computes their
...
class TypedProtoDecoderBase : public ProtoDecoder {
 public:
  ...
  inline const Field& GetSingle(uint32_t id) {
    return PERFETTO_LIKELY(id <= capacity_) ? fields_[id] : fields_[0];
  }
 ...
 protected:
  ...
  // Points to the storage. Either on-stack (provided by the template
  // specialization) or |heap_storage_| in case of a large number of repeated
  // fields.
  Field* fields_;

  // Number of active |fields_| entries. This is initially equal to the highest
  // number of fields for the message (MAX_FIELD_ID) and can grow up to
  // |capacity_| in the case of repeated fields.
  uint32_t size_;

  // Initially equal to kFieldsCapacity of the TypedProtoDecoderTemplate
  // specialization. Can grow when falling back on heap-based storage, in which
  // case it represents the number of elements in the |heap_storage_| array.
  uint32_t capacity_;
};

// In the case of non-repeated fields, this constant defines the highest field
// id we are able to decode. This limit is to limit the on-stack storage.
// In the case of repeated fields, this constant defines the max number of
// repeated fields that we'll be able to store before falling back on the heap.
constexpr size_t kMaxDecoderFieldId = 999;

template <int MAX_FIELD_ID, bool HAS_REPEATED_FIELDS>
class TypedProtoDecoderTemplate : public TypedProtoDecoderBase {
 public:
  TypedProtoDecoderTemplate(const uint8_t* buffer, uint64_t length)
