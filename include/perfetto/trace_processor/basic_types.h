...
class Status {
 public:
  Status() = default;
  Status(std::string error) : message_(std::move(error)) {}
  ...
  // Only valid to call when this message has an Ok status (i.e. ok() returned
  // true or operator bool() returned false).
